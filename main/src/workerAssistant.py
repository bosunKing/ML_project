...
# - 일단 이미지는 5초 디스플레이하고 넘어가게/ 영상은 2회 반복재생되면 넘어가게함 
    ...
    def object_grab_check(self, x1,x2,y1,y2):
        
        self.hand_xy_flag = False
        # hand_landmark_list = [4, 8]  # 손 끝점
        hand_point = 4 
        error = 5  # 오차
        # 좌표 리스트의 각 좌표에 오차를 적용하여 사각형의 좌표를 계산
        rect = [(x1 - error, y1 + error), 
                (x2 + error, y1 + error), 
                (x2 + error, y2 - error), 
                (x1 - error, y2 - error)]
        ...
        if rect :
            if self.mp_results.multi_hand_landmarks:
                for hand_landmarks in self.mp_results.multi_hand_landmarks: 
                        self.hand_xy_flag = cv2.pointPolygonTest(rect, (hand_landmarks.landmark[hand_point].x, hand_landmarks.landmark[hand_point].y), False) 
    ...
    def yolo_update_workview(self, frame_for_yolo):
        ...
        # print("---")
    def yolo_update_materialview(self, frame_for_yolo): #materialview
            ...
            if self.xml_detection_modellist[self.current_index] == '0': 
                ...
                for r in results:
                    for idx,cls_name in enumerate(r.boxes.cls):
                        tmp_name = names[int(cls_name.item())]
                        if tmp_name == 'bar':
                            size = self.measure_bar_size(r.boxes.xyxy.cpu()[idx])
                            if size != 0:
                                tmp_name += str(size)
    ...
    def draw_rec_workview(self,img_form):
    ...
    def draw_rec_materialview(self,img_form):
        result = img_form.copy()
        if self.is_grabbing:         # True ,False 반환
                if  self.hand_xy_flag:
                    self.grab_text = "Good."
                else:
                    self.grab_text = "Grab in red box !"
        else:
            self.grab_text = "Grab in red box !"
        ...
        for val in self.yolo_detect_class_coordinate:
            for idx, coord in enumerate(val):
                ...
                if self.yolo_detect_class[idx] in self.xml_detection_partlist[self.current_index]:
                    cv2.rectangle(result, (int(x1.item()), int(y1.item())), (int(x2.item()), int(y2.item())), (255, 0, 0), 2)
                    self.object_grab_check(int(x1.item()), int(y1.item()), int(x2.item()), int(y2.item()))
                ...
                if self.yolo_detect_class[idx] in self.xml_detection_partlist[self.current_index]:
                    ...
                    cv2.putText(result, self.grab_text, (200, 50), font, font_scale2, font_color3, font_thickness2)           #위치 수정 필요
        return result 
...
# --- workGuideLabel 띄우기 끝 
    ...
    def update_frame2(self):
        ret, frame_2 = cap2.read()  # 웹캠 2번 material
        if ret:
            frame_2 = cv2.rotate(frame_2, cv2.ROTATE_180)
            frame_2 = cv2.cvtColor(frame_2, cv2.COLOR_BGR2RGB)
            self.mp_update(frame_2)
            self.yolo_update_materialview(frame_2)
            frame_2 = self.draw_rec_materialview(frame_2)
